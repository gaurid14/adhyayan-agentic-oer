"""
blockchain/deploy_evaluation_scores.py
------------------------------------
One-shot deployment script for EvaluationScores.sol.
"""
...
SOL_PATH = os.path.join(BASE_DIR, "contract", "EvaluationScores.sol")
ABI_OUT  = os.path.join(BASE_DIR, "abi.json")
...
print("Compiling EvaluationScores.sol...")
compiled = compile_source(source, solc_version="0.8.19")
contract_id, contract_interface = next(
    (k, v) for k, v in compiled.items() if "EvaluationScores" in k
)
...
print(f"  Update CONTRACT_ADDRESS in blockchain/services/evaluation_scores.py with: {address}")
print("="*60)
